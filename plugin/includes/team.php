...
function toyno_team_members_query( array $args = [] ) {
	...
	if( !empty( $args['type'] ) ) {
		...
		$meta[] = [
			'key'		=> 'toyno_team_member_type',
			'value'		=> explode(',', $args['type']),
			'compare'	=> 'IN'
		];
...
function toyno_team_members( $args ) {
	...
	$query = toyno_team_members_query( $args );
	...
	if( $query->have_posts() ) {
		...
		foreach( $query->posts as $post ) {
			...
			$html[] = toyno_team_member( ['id' => $post->ID] );
		...
		$atts = [
			...
			'data-layout' => 'grid',
			'data-member-type' => $args['type']
		];

		if( !empty( $args['label'] ) ) {

			$atts['data-label'] = $args['label'];

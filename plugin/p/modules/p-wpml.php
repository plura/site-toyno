...
add_filter('body_class', function( $classes ) {
	...
	if( is_singular() && class_exists('sitepress') ) {
		...
		$c[] = 'wpmlobj-id-' . p_wpml_id( get_the_ID() );
...
function p_wpml_featured_image( $postID, $acf_field = false, $size = 'large' ) {

	$id = p_wpml_featured_image_id( $postID );
...
//if no post thumbnail is found, it searches an acf field
function p_wpml_featured_image_id( $postID, $acf_field = false ) {
...
function p_wpml_id( $id = false, $default = true, $type = 'post' ) {
    ...
    if( p_wpml() && ( !$default || is_string( $default ) || $sitepress->get_current_language() !== $sitepress->get_default_language() ) ) {
 ...
 */
function p_wpml_query( $query_args, $type = false ) {

	global $sitepress;

	if( p_wpml() && $sitepress->get_current_language() !== $sitepress->get_default_language() ) {
...
// ESSENTIAL GRID
function p_wpml_essential_grid($posts, $alias, $label = false) {

    $ids = [];

    foreach( $posts as $post ) {

        $ids[] = p_wpml_id( $post->ID );
